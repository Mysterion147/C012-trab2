...
# Variáveis globais de ingredientes e semáforos de contagem
...
def verificar_ingredientes(ingrediente, quantidade):
    ...
    global ingredientes, semaforos
    
    # Tenta adquirir o semáforo para todos os itens necessários
    for _ in range(quantidade):
        if not semaforos[ingrediente].acquire(blocking=False):
            # Caso não consiga adquirir todos, libera os já adquiridos e retorna False
            for _ in range(_):
                semaforos[ingrediente].release()
            return False
    
    # Caso tenha adquirido todos, decremente a quantidade usada do estoque
...
def processar_pedidos(queue):
    while True:
        ...
        if verificar_ingredientes('carne', 1) and \
            ...
            # Libera os semáforos após o uso
            semaforos['carne'].release()
            semaforos['queijo'].release()
            semaforos['tomate'].release()
        else:
